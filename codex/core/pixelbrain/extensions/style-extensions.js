...
export const styleGameBoy = {
  ...
  hooks: {
    ...
    onColorByte(palette, _context) {
      return {
        ...palette,
        colors: this.config.palette,
        byteMap: this.config.palette.reduce((map, color, i) => {
          map[String(i)] = color;
          return map;
        }, {}),
      };
    ...
    onNoiseGen(noise, _context) {
      if (!this.config.dithering) return noise;
      return applyOrderedDither(noise, this.config.palette.length);
    },
    ...
    onRender(buffer, _context) {
      ...
      const palette = this.config.palette.map(hex => {
...
export const style8Bit = {
  ...
  hooks: {
    ...
    onRender(buffer, _context) {
      const result = new Uint8ClampedArray(buffer);
      const { colorCount, scanlines } = this.config;
...
export const styleCRT = {
  ...
  hooks: {
    onRender(buffer, _context) {
      const result = new Uint8ClampedArray(buffer);
      const { scanlineIntensity, curvature, phosphorGlow } = this.config;
